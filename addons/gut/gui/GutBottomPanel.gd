...
var TestScript = preload('../test.gd')
var GutConfigGui = preload('gut_config_gui.gd')
var ScriptTextEditors = preload('script_text_editor_controls.gd')

var _interface = null;
var _is_running = false;
var _gut_config = preload('../gut_config.gd').new()
...
func _ready():
    ...
    var check_import = preload('../images/red.png')
...
func _is_test_script(script):
    ...
    while(from and from.resource_path != '../test.gd'):
...
func _run_tests():
    ...
    _interface.play_custom_scene('GutRunner.tscn')
    _is_running = true
