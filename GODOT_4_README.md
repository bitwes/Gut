...
```
* Doubling no longer supports paths to a script or scene.  Load the script or scene first and pass that to `double`.  Error messages are printed to indicate the old way is being used and null will be returned.  This will most likely result in the following error when you try to instantiate your double:
```
