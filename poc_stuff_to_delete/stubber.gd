...
func _get_hash_key_for_object(obj, method):
	var key = _get_path_from_variant(obj)
...
func is_stubbed(obj, method):
	var key = _get_hash_key_for_object(obj, method)
	return returns.has(key) and returns[key].has(method)
