...
func test_can_make_parsed_script():
	var ps = SingletonParser.GutParsedSingleton.new(OS)
	assert_not_null(ps)


func test_parsed_singleton_populates_methods():
	var ps = SingletonParser.GutParsedSingleton.new(Time)
...
func test_parsed_singleton_has_signals():
	var ps = SingletonParser.GutParsedSingleton.new(AudioServer)
...
func test_can_parse_all_singletons(p = use_parameters(GutUtils.GodotSingletons.class_ref)):
	var ps = SingletonParser.GutParsedSingleton.new(p)
	assert_not_null(ps)

func test_constants_are_added_as_enums():
	var ps = SingletonParser.GutParsedSingleton.new(DisplayServer)
